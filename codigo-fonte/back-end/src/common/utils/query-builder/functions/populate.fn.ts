...
const populateAddSelectFieldsAndFilter = (
    select,
    ...
    if (populate[index]?.select) {
        ...
        if (populate[index]?.populate?.length) {
            populate[index].populate.forEach(
                (populateInside: PopulateFields, indexInside: number) => {
                    populateAddSelectFieldsAndFilter(
                        select[value.path]['select'],
