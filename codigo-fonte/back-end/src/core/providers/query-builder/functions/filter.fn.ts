...
export const filter = (query) => {
    ...
    if (query.filter) {
        ...
        filter.forEach((value: FilterFields) =>
            whereAddFilters(value, query['where']),
        );
    }
...
const whereAddFilters = (value: FilterFields, where) => {
    ...
    const insensitive =
        value.insensitive === true ? { mode: 'insensitive' } : undefined;
    ...
    if (value?.filterGroup) {
        if (value.operator) {
            where[value.filterGroup.toUpperCase()]?.push({
                [value.path]: { [value.operator]: value.value, ...insensitive },
            });
        } else {
            where[value.filterGroup.toUpperCase()]?.push({
    ...
    if (value?.filter?.length) {
        ...
        value.filter.forEach((filter) => {
            if (filter.filterInsideOperator) {
                if (!where[value.path][filter.filterInsideOperator])
                    where[value.path][filter.filterInsideOperator] = {};
