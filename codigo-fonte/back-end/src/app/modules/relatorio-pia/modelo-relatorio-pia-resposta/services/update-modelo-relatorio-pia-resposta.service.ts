...
export class UpdateModeloRelatorioPiaRespostaService {
    ...
    async execute(uid: string, data: UpdateModeloRelatorioPiaRespostaDto): Promise<ModeloRelatorioPiaResposta | null> {
        const alreadyExists = await this.modeloRelatorioPiaRespostaRepository.findByUid(uid);

        if (!alreadyExists) {
            throw new AppError("Modelo de relatório PIA não encontrado");
        }

        const modeloRelatoriaPiaPergunta = await this.modeloRelatorioPiaRespostaRepository.update(data, alreadyExists)
