...
export class UpdateModeloRelatorioPiaRespostaController {
    constructor(private updateModeloRelatorioPiaResposta: UpdateModeloRelatorioPiaRespostaService) {}
    
    @Patch(':uid')
    @ApiOperation({ summary: 'Atualiza um modelo de resposta pelo UID' })
    @ApiBody({ type: UpdateModeloRelatorioPiaRespostaDto,
    ...
    @ApiResponseError()
    async handle(@Param('uid') uid: string, @Body() data: UpdateModeloRelatorioPiaRespostaDto): Promise<void> {
