...
export class FindUidModeloRelatorioPiaRespostaController {
    constructor(private findUidModeloRelatorioPiaRespostaService: FindUidModeloRelatorioPiaRespostaService) {}
    ...
    @ApiResponseError()
    async handle(@Param('uid') uid: string): Promise<ModeloRelatorioPiaResposta | null> {
        const modeloRelatorioPiaResposta = await this.findUidModeloRelatorioPiaRespostaService.execute(uid);
