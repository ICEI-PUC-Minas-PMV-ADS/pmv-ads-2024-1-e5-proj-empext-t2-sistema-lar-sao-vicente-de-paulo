...
export class CreateModeloRelatorioPiaRespostaController {
    constructor(private createModeloRelatorioPiaResposta: CreateModeloRelatorioPiaRespostaService) {}
    
    @Post()
    @ApiOperation({ summary: 'Cria um modelo de resposta para o modelo de pergunta' })
    @ApiBody({
    ...
    @ApiResponseError()
    async handle(@Body() data: CreateModeloRelatorioPiaRespostaDto): Promise<ModeloRelatorioPiaResposta> {
        const modeloRelatorioPiaResposta = await this.createModeloRelatorioPiaResposta.execute(data);
