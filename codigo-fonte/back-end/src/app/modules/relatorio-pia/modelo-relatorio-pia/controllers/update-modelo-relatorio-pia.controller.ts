...
export class UpdateModeloRelatorioPiaController {
    constructor(private updateModeloRelatorioPia: UpdateModeloRelatorioPiaService) {}
    
    @Patch(':uid')
    @ApiOperation({ summary: 'Atualiza um modelo de relatório PIA pelo UID' })
    @ApiBody({ type: UpdateModeloRelatorioPiaDto,
    ...
    @ApiResponseError()
    async handle(@Param('uid') uid: string, @Body() data: UpdateModeloRelatorioPiaDto): Promise<void> {
