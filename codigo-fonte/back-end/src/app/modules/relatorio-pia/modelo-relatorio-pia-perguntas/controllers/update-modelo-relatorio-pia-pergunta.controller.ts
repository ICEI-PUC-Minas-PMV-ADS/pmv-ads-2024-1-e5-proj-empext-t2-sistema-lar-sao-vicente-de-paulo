...
export class UpdateModeloRelatorioPiaPerguntaController {
    constructor(private updateModeloRelatorioPiaPergunta: UpdateModeloRelatorioPiaPerguntaService) {}
    
    @Patch(':uid')
    @ApiOperation({ summary: 'Atualiza um modelo de pergunta pelo UID' })
    @ApiBody({ type: UpdateModeloRelatorioPiaPerguntaDto,
    ...
    @ApiResponseError()
    async handle(@Param('uid') uid: string, @Body() data: UpdateModeloRelatorioPiaPerguntaDto): Promise<void> {
