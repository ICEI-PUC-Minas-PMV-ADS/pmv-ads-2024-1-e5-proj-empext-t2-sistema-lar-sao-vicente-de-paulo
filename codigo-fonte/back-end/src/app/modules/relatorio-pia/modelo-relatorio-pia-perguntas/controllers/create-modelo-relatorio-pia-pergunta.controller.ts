...
export class CreateModeloRelatorioPiaPerguntaController {
	constructor(
		private createModeloRelatorioPiaPergunta: CreateModeloRelatorioPiaPerguntaService,
	) {}

	@Post()
	@Roles(RoleModeloRelatorioPia.CREATE)
	@ApiOperation({
		summary: 'Cria um modelo de pergunta para o modelo relatório PIA',
	})
	@ApiBody({
	...
	@ApiResponseError()
	async handle(
		@Body() data: CreateModeloRelatorioPiaPerguntaDto,
	): Promise<ModeloRelatorioPiaPergunta> {
		const modeloRelatorioPiaPergunta =
			await this.createModeloRelatorioPiaPergunta.execute(data);
